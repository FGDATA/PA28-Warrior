...
# Changes for FlightGear PA28 Navigator by Joshua Davidson (it0uchpods)
##
...
setprop("/autopilot/KAP140/settings/target-pressure-rate-fpm", 0);
...
var ptCheck = func {
  ...
  else
  {
    ...
    if (elevatorControl.getValue() < -0.05)
    {
    ...
    elsif (elevatorControl.getValue() > 0.05)
    {
         ...
         altCaptured == 0)
  {
    ...
    if (altDifference < 200)
    {
      ...
      if (altDifference < 5)
      {
        #print("altCapture()");
        altCaptured = 1;
        setprop("/autopilot/KAP140/locks/alt-hold/", 1);
